...
class PostProcessor:
    ...
    def run(self):
        ...
        if self.haddFileName:
            # Search for haddnano.py in multiple locations
            haddnano = None
            if os.path.isfile("./haddnano.py"):
                haddnano = "./haddnano.py"
            else:
                # Try to find it in the NanoAODTools scripts directory
                script_dir = os.path.join(os.path.dirname(__file__), "..", "..", "analysis", "scripts", "haddnano.py")
            ...
            print(f"[PostProcessor] Merging {len(outFileNames)} files into {self.haddFileName} using {haddnano}")
            ret = os.system("%s %s %s" %
                      (haddnano, self.haddFileName, " ".join(outFileNames)))
            if ret != 0:
                print(f"[PostProcessor] WARNING: haddnano.py failed with return code {ret}")
            else:
                print(f"[PostProcessor] Successfully merged files into {self.haddFileName}")
        if self.jobReport:
